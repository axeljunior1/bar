...
import { SlateDetailClient } from "@/components/slates/SlateDetailClient";
...
export default async function SlateDetailPage({ params }: SlateDetailPageProps) {
  ...
  const [{ data: slate }, catalog, lines, paymentMethods] = await Promise.all([
    supabase
      .from("slates")
      ...
      {isOpen ? (
        <>
          ...
          <SlateDetailClient
            ...
            clientName={slate.client_name}
            initialLines={lines}
            initialTotal={Number(slate.total)}
        ...
        <>
          ...
          <section className="mb-8 space-y-4">
            ...
            <SlateLinesList lines={lines} readOnly />
          </section>
        </>
      )}
    </AppShell>
