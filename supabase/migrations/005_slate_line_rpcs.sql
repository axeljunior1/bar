-- RPC caisse : mutations ardoise en un seul round-trip

CREATE OR REPLACE FUNCTION public.recalculate_slate_total_internal(
  p_slate_id uuid,
  p_bar_id uuid
)
RETURNS numeric
...
CREATE OR REPLACE FUNCTION public.delete_slate_line(
  p_slate_id uuid,
  p_line_id uuid
)
RETURNS jsonb
...
BEGIN
  ...
  DELETE FROM public.slate_lines
  WHERE id = p_line_id
    AND slate_id = p_slate_id
    AND bar_id = v_bar_id;
...
GRANT EXECUTE ON FUNCTION public.add_slate_line(uuid, uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_slate_line_quantity(uuid, uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_slate_line(uuid, uuid) TO authenticated;
